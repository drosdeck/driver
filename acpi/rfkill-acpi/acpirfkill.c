...
bool rfkill_inited;
static int positivo_rfkill_set(void *data, bool blocked)
{
	acpi_status status;
	u32 cap = (unsigned long)data;
	printk("Valor do cappp %u\n",cap);
	if(rfkill_inited){
...
static const struct rfkill_ops positivo_rfkill_ops = {
	.set_block = positivo_rfkill_set,
};

static struct rfkill *positivo_rfkill_register(struct device *dev, enum rfkill_type type, char *name, u32 cap)
{
	int err;
	...
	acpi_status status;
          ...
          rfkill_dev = rfkill_alloc(name,dev,type, &positivo_rfkill_ops, (void *)(unsigned long)cap);
	return rfkill_dev;
}

static int positivo_rfkill_init(struct device *dev)
{
   //	wireless_rfkill =
return 0;
}
...
static int positivo_platform_probe(struct platform_device *device)
{
    ...
    err = positivo_rfkill_init(&device->dev);	
